...
Durante a execução da atividade, a principal dificuldade encontrada não esteve no Docker em si, mas sim no uso do terminal Windows PowerShell. Tive desafios com a sintaxe do comando `curl`, pois o PowerShell o interpreta como um "alias" para o `Invoke-WebRequest`, o que causou erros de formatação de JSON e cabeçalhos. Aprendi que para usar o `curl` original no Windows, é preciso chamar `curl.exe` e escapar as aspas duplas com contra-barras (`\"`) ou crases (`` \` ``). Além disso, aprendi que comandos no PowerShell são encadeados com ponto-e-vírgula (`;`) e não com `&&`. Em relação à tecnologia estudada, a prática consolidou muito bem o conceito de efemeridade dos contêineres e a extrema importância do uso de volumes (`-v`) para qualquer serviço de banco de dados ou arquivos que precise guardar estado em produção.